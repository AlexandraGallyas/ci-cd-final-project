...
def create_counters(name):
    ...
    if name in COUNTER:
        my_message = f"Counter {name} already exists"
        return abort(status.HTTP_409_CONFLICT, my_message)
...
def read_counters(name):
    ...
    if name not in COUNTER:
        my_message = f"Counter {name} does not exist"
        return abort(status.HTTP_404_NOT_FOUND, my_message)
